...
# --- Model 4: Advanced Model (Smartphone Sensor Self-Attention Network w/ SwiGLU) ---
class SwiGLU(nn.Module):
    def forward(self, x):
        x, gate = x.chunk(2, dim=-1)
        return F.silu(gate) * x

class SmartSANN(nn.Module):
    def __init__(self, input_dim, num_classes):
        super(SmartSANN, self).__init__()
        self.feature_proj = nn.Linear(input_dim, 256)
        self.attention = nn.MultiheadAttention(embed_dim=256, num_heads=8, batch_first=True)
        self.norm1 = nn.LayerNorm(256)
        
        self.ffn_proj1 = nn.Linear(256, 512 * 2) 
        self.swiglu = SwiGLU()
        self.ffn_proj2 = nn.Linear(512, 256)
    ...
    def forward(self, x):
        h = self.feature_proj(x).unsqueeze(1)
        attn_out, _ = self.attention(h, h, h)
        h = self.norm1(h + self.dropout(attn_out))
        
        ffn_in = h
        ffn_out = self.ffn_proj1(ffn_in)
        ffn_out = self.swiglu(ffn_out)
...
def plot_results(test_loss_mlp, test_loss_cnn, test_loss_lstm, test_loss_adv):
    plt.figure(figsize=(10, 6))
    plt.plot(test_loss_mlp, label='ANN (MLP) Test Loss', linestyle='--')
    plt.plot(test_loss_cnn, label='1D-CNN Test Loss', linestyle='-.')
    plt.plot(test_loss_lstm, label='RNN (LSTM) Test Loss', linestyle=':')
    plt.plot(test_loss_adv, label='SmartSANN Test Loss', linewidth=2)
...
def plot_confusion_matrix(model, test_loader, activity_mapping, filename='confusion_matrix.png'):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model.eval()
    ...
    plt.title("Confusion Matrix - SmartSANN")
...
def main():
    ...
    print("\n--- Training Model 4: Advanced Model (SmartSANN) ---")
    adv_model = SmartSANN(input_dim=561, num_classes=6)
    _, adv_test_loss, _ = train_model(adv_model, train_loader, test_loader, epochs=50)
    
    print("\nGenerating Plots...")
    plot_results(mlp_test_loss, cnn_test_loss, lstm_test_loss, adv_test_loss)
    true_labels, pred_labels = plot_confusion_matrix(adv_model, test_loader, activity_mapping)
